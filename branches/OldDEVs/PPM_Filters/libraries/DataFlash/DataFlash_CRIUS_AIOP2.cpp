...
/*
	DataFlash_CRIUS_AIOP2.cpp - DataFlash log library for AT45DB161
...
void dataflash_CS_inactive_crius()
{
  digitalWrite(DF_SLAVESELECT,HIGH); //disable device
}

void dataflash_CS_active_crius()
{
  digitalWrite(DF_SLAVESELECT,LOW); //enable device
}

// Constructors ////////////////////////////////////////////////////////////////
DataFlash_CRIUS_AIOP2::DataFlash_CRIUS_AIOP2()
{
}
...
void DataFlash_CRIUS_AIOP2::ReadManufacturerID()
{
  dataflash_CS_active_crius();     // activate dataflash command decoder
  ...
  dataflash_CS_inactive_crius();    // Reset dataflash command decoder
}


bool DataFlash_CRIUS_AIOP2::CardInserted(void)
{
    return true;
}

// Read the status register
byte DataFlash_CRIUS_AIOP2::ReadStatusReg()
{
  byte tmp;
...
inline
uint16_t DataFlash_CRIUS_AIOP2::PageSize()
{
...
void DataFlash_CRIUS_AIOP2::PageToBuffer(unsigned char BufferNum, uint16_t PageAdr)
{
  dataflash_CS_active_crius();     // activate dataflash command decoder
  ...
  dataflash_CS_inactive_crius();	//initiate the transfer
  dataflash_CS_active_crius();

  while(!ReadStatus());  //monitor the status register, wait until busy-flag is high

  dataflash_CS_inactive_crius();

}

void DataFlash_CRIUS_AIOP2::BufferToPage (unsigned char BufferNum, uint16_t PageAdr, unsigned char wait)
{
  dataflash_CS_active_crius();     // activate dataflash command decoder
  ...
  dataflash_CS_inactive_crius();	//deactivate dataflash command decoder
}

void DataFlash_CRIUS_AIOP2::BufferWrite (unsigned char BufferNum, uint16_t IntPageAdr, unsigned char Data)
{
  dataflash_CS_active_crius();     // activate dataflash command decoder
  ...
  dataflash_CS_inactive_crius();   // disable dataflash command decoder
}

unsigned char DataFlash_CRIUS_AIOP2::BufferRead (unsigned char BufferNum, uint16_t IntPageAdr)
{
  byte tmp;
...
void DataFlash_CRIUS_AIOP2::PageErase (uint16_t PageAdr)
{
  dataflash_CS_active_crius();     // activate dataflash command decoder
  ...
  dataflash_CS_inactive_crius();               //initiate flash page erase
  dataflash_CS_active_crius();
  while(!ReadStatus());
...
void DataFlash_CRIUS_AIOP2::BlockErase (uint16_t BlockAdr)
{
  dataflash_CS_active_crius();     // activate dataflash command decoder
...
void DataFlash_CRIUS_AIOP2::ChipErase(void (*delay_cb)(unsigned long))
{
